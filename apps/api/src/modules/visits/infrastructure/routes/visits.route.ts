...
function parseRecordVisitBody(body: unknown) {
  ...
  return { facilityId: parsed.data.facilityId, visitedAt: parsed.data.visitedAt ? new Date(parsed.data.visitedAt) : undefined };
}
...
const weeklySummaryRoute = new Elysia()
  ...
  .get(
    ...
    { detail: { summary: "Get current weekly clinic visit summary", tags: ["Visits"], security: [{ bearerAuth: [] }] } }
  );
