...
function groupClinicsByTerritoryId(
  ...
  const grouped = new Map<string, Array<{ id: string; name: string; lat: number; lng: number }>>()
...
export class TerritoryCoverageUseCases {
  ...
  async getAnalyticsView(input: { groupingTerritoryId: string; scope: ScopeContext }) {
    const grouping = await this.assertReadableGrouping(input.groupingTerritoryId, input.scope)
  ...
  private async resolveScopedRepPatchIds(scope: ScopeContext): Promise<string[]> {
    if (scope.isGlobal) {
      return (await this.deps.territoryRepository.findActiveByTypeSlug('patch')).map(
  ...
  private async assertReadableGrouping(groupingTerritoryId: string, scope: ScopeContext) {
    ...
    if (!scope.isGlobal) {
      await assertManagerReadableTerritory(scope, groupingTerritoryId, this.deps.closureRepository)
    }
