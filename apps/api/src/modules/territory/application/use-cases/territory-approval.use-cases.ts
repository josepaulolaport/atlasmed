...
import type { TerritoryCrudUseCases } from './territory-crud.use-cases'
...
export class TerritoryApprovalUseCases {
  ...
  async approveRequest(input: { requestId: string; reviewerId: string; note?: string }) {
    ...
    if (request.status !== 'pending') {
      throw new OperationNotAllowedError('approve_request', 'Only pending requests can be approved')
    }
  ...
  async rejectRequest(input: { requestId: string; reviewerId: string; note?: string }) {
    ...
    if (request.status !== 'pending') {
      throw new OperationNotAllowedError('reject_request', 'Only pending requests can be rejected')
    }
  ...
  private async applyApprovedRequest(request: {
    ...
    switch (request.type) {
      ...
      case 'clinic_territory_change': {
        ...
        if (!target?.isActive) {
          throw new OperationNotAllowedError('approve_request', 'Target territory must be active')
        }
