...
import { assertBoundaryProvidedForType } from './territory-boundary.application'
...
describe('assertBoundaryProvidedForType', () => {
  ...
  it('requires a boundary when the type supports boundaries', () => {
    expect(() => assertBoundaryProvidedForType(true, undefined)).toThrow(OperationNotAllowedError)
  ...
  it('rejects a boundary when the type does not support boundaries', () => {
    expect(() => assertBoundaryProvidedForType(false, polygon)).toThrow(OperationNotAllowedError)
  })
