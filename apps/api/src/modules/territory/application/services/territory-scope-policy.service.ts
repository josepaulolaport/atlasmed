...
export async function resolveReadableTerritoryIds(
  ...
  const ancestors = await closureRepository.findAncestorIds(scope.effectiveTerritoryIds)
...
export function assertTerritoryReadable(
  ...
  if (!readableTerritoryIds.has(territoryId)) {
    throw new OperationNotAllowedError('read_territory', 'Territory is outside your readable scope')
  }
}

export async function assertLeafTerritoryInJurisdiction(input: {
  scope: Pick<ScopeContext, 'isGlobal' | 'effectiveTerritoryIds'>
  ...
  assertTerritorialJurisdiction(input.scope, input.territoryId, input.operation)
  ...
  const activeChildCount = await input.territoryRepository.countActiveChildren(input.territoryId)
...
export async function assertManagerTerritoryApprovalRequest(input: {
  ...
  switch (input.type) {
    case 'create_territory': {
      const parentId =
        (typeof payload.parentId === 'string' ? payload.parentId : undefined) ?? undefined
      ...
      const readableIds = await resolveReadableTerritoryIds(input.scope, input.closureRepository)
    ...
    case 'reparent_territory': {
      if (!input.targetTerritoryId) {
        throw new OperationNotAllowedError('reparent_territory', 'Target territory is required')
      }
      ...
      if (typeof newParentId === 'string' && newParentId.length > 0) {
        assertTerritorialJurisdiction(input.scope, newParentId, 'reparent_territory')
      }
    ...
    case 'deactivate_territory': {
      if (!input.targetTerritoryId) {
        throw new OperationNotAllowedError('deactivate_territory', 'Target territory is required')
      }
    ...
    default:
      throw new OperationNotAllowedError('submit_approval', 'Unsupported territory approval type')
  }
}
