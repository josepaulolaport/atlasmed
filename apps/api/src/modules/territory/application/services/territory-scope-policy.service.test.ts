...
describe('TerritoryScopePolicyService', () => {
  ...
  it('checks territorial jurisdiction', () => {
    expect(isInTerritorialJurisdiction(scopedManager, 'patch-in-scope')).toBe(true)
  ...
  it('resolves readable ids with ancestors', async () => {
    ...
    const readable = await resolveReadableTerritoryIds(scopedManager, closureRepository)

    expect(readable).toEqual(new Set(['patch-in-scope', 'region-1', 'country-1']))
  })
  ...
  it('skips jurisdiction checks for global scope', () => {
    expect(() =>
      assertTerritorialJurisdiction({ isGlobal: true, effectiveTerritoryIds: [] }, 'any', 'test')
    ).not.toThrow()
  })
})
