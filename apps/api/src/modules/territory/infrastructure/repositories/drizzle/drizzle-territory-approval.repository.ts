import { territoryApprovalRequests, type territoryApprovalStatusEnum } from '@atlasmed/database'
...
export class DrizzleTerritoryApprovalRepository implements TerritoryApprovalRepository {
  ...
  async list(params: {
    ...
    const where = params.status ? eq(territoryApprovalRequests.status, params.status) : undefined
    ...
    const [items, countRows] = await Promise.all([
      db
        ...
        .offset(skip)
        .limit(params.limit),
      db.select({ count: sql<number>`count(*)` }).from(territoryApprovalRequests).where(where)
    ])
