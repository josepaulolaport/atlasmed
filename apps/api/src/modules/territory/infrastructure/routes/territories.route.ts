...
import { InsufficientPermissionsError, ResourceNotFoundError } from '../../../../shared/errors'
import { auth } from '../../../access/composition'
import { requirePermission } from '../../../access/infrastructure/middleware/permission.middleware'
import {
  assertManagerReadableTerritory,
  isAdminRole,
  isManagerRole
} from '../../application/use-cases/territory-crud.use-cases'
...
export const territoriesRoute = new Elysia()
  ...
  .get(
    ...
    async ({ query, getScope }) => {
      ...
      return territoryUseCases
        .listTerritories()
        .listTerritories(query.format === 'tree' ? 'tree' : 'flat', scope, {
          typeSlug: query.type,
    ...
    },
    {
      query: t.Object({
        ...
        type: t.Optional(
          t.String({ description: 'Filter by territory type slug (e.g. manager_zone, patch)' })
        ),
        managerTerritoryId: t.Optional(
          t.String({ description: 'Filter patches by manager zone territory ID' })
        ),
  ...
  .get('/territories/:id', async ({ params, getScope }) => {
    ...
    if (!scope.isGlobal) {
      await assertManagerReadableTerritory(scope, params.id, territoryRepositories.closure)
    }
    return territory
  })
  .use(requirePermission('read', 'TERRITORY'))
