...
export class VerificationService {
  ...
  async changeEmail(params: { userId: string; newEmail: string; token: string }): Promise<void> {
    const result = await this.verifyToken({
      token: params.token,
      type: 'EMAIL_CHANGE',
      userId: params.userId
  ...
  async changePhone(params: { userId: string; newPhone: string; token: string }): Promise<void> {
    const result = await this.verifyToken({
      token: params.token,
      type: 'PHONE_CHANGE',
      userId: params.userId
