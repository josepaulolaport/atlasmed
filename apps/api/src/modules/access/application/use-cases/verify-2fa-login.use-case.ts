...
export class Verify2FALoginUseCase {
  ...
  async execute(params: {
    ...
    const pendingLogin = await this.deps.pending2faLoginService.get(params.pendingToken)
    ...
    if (!valid) {
      const attemptsRemaining = await this.deps.pending2faLoginService.recordFailedAttempt(
        params.pendingToken
      )
      ...
      await this.deps.auditLog.logFailedLoginAttempt({
        ...
        reason: attemptsRemaining ? 'invalid_2fa_code' : 'invalid_2fa_code_max_attempts',
