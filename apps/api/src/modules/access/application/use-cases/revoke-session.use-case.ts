...
export class RevokeSessionUseCase {
  ...
  async execute(input: RevokeSessionInput): Promise<RevokeSessionOutput> {
    ...
    if (currentSession && sessionsMatchSameDevice(session, currentSession)) {
      return { success: false, error: 'cannot-revoke-current' }
    }

    const revokedSessionIds = await this.dependencies.sessionRepository.revokeAllActiveForDevice(
      session.userId,
      {
        id: session.id,
