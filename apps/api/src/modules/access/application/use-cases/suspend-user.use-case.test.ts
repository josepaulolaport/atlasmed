...
describe('SuspendUserUseCase', () => {
  ...
  describe('user not found', () => {
    it('should throw error when user not found', async () => {
      ...
      await expect(suspendUserUseCase.execute(adminSuspendParams('non-existent'))).rejects.toThrow(
        'User not found'
      )
    })
  ...
  describe('already suspended user', () => {
    it('should throw error when user is already suspended', async () => {
      ...
      await expect(suspendUserUseCase.execute(adminSuspendParams('user-123'))).rejects.toThrow(
