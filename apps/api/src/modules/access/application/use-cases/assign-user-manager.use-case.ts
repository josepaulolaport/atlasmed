...
export class AssignUserManagerUseCase {
  ...
  async execute(params: {
    ...
    if (params.actorRole !== Role.ADMIN) {
      throw new InsufficientPermissionsError(['user:assign_manager'], [`role:${params.actorRole}`])
    }
    ...
    if (params.targetUserId === params.managerId) {
      throw new OperationNotAllowedError('assign_manager', 'A user cannot manage themselves')
    }
