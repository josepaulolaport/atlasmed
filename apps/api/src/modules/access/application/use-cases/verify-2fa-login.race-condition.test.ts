...
import { createMockSessionCache, createMockUserRepository } from '../../test-helpers/fixtures'
...
describe('Verify2FALoginUseCase race handling', () => {
  ...
  it('creates at most one session for concurrent successful verifications', async () => {
    ...
    expect(
      rejected.some(
        (result) => result.status === 'rejected' && result.reason instanceof TokenInvalidError
      )
    ).toBe(true)
  })
})
