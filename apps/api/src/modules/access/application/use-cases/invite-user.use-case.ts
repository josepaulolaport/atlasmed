...
export class InviteUserUseCase {
  ...
  async execute(params: InviteUserParams) {
    return tracer.with('user.invite', async () => this.executeInvite(params), {
      'user.id': params.invitedByUserId,
      'app.module': 'access'
    })
  }
  ...
  private async executeInvite(params: InviteUserParams) {
    ...
    const inviterRole = inviter.role as { name: string; priority?: number | null } | undefined
    ...
    if (inviterRole.name === Role.MANAGER && role.name !== Role.REP) {
      throw new InsufficientPermissionsError([`role:${role.name}`], [`role:${Role.MANAGER}`])
    }
    ...
    this.deps.metrics.recordInvite(params.email ? 'email' : 'phone', 'create')
