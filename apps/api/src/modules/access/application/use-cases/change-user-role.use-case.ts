import { canChangeUserRole, type Role } from '@atlasmed/access'
import {
...
export class ChangeUserRoleUseCase {
  ...
  async execute(params: { targetUserId: string; newRoleId: string; changedBy: string }) {
    ...
    const actorRole = actor.role as { name: string; priority?: number | null } | undefined
    const targetRole = target.role as { name: string; priority?: number | null } | undefined
    ...
    if (!canChangeUserRole(actorRole.name as Role)) {
      throw new InsufficientPermissionsError(['role:change'], [`role:${actorRole.name}`])
    }
