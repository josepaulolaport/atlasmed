...
describe('Confirm2FASetupUseCase', () => {
  ...
  it('should enable 2FA with valid code', async () => {
    ...
    expect(mockRevokeAllByUserId).toHaveBeenCalledWith('user-123', 'session-current')
    expect(mockSessionCache.invalidateByUserId).toHaveBeenCalledWith('user-123', 'session-current')
  })

  it('should reject invalid TOTP code', async () => {
    mockTwoFactorService.verifyTotp = mock(async () => false)

    await expect(useCase.execute({ userId: 'user-123', code: '000000' })).rejects.toThrow(
      InvalidCredentialsError
    )
  })
  ...
  it('should reject when setup expired', async () => {
    ...
    await expect(useCase.execute({ userId: 'user-123', code: '123456' })).rejects.toThrow(
      ValidationError
    )
  })
  ...
  it('should reject when user not found', async () => {
    ...
    await expect(useCase.execute({ userId: 'missing', code: '123456' })).rejects.toThrow(
      UserNotFoundError
    )
  })
})
