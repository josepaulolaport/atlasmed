...
describe('Setup2FAUseCase', () => {
  ...
  it('should return secret and otpauth URL', async () => {
    ...
    expect(mockTwoFactorService.storePendingSetup).toHaveBeenCalledWith('user-123', 'BASE32SECRET')
  })
  ...
  it('should reject when 2FA is already enabled', async () => {
    ...
    await expect(useCase.execute({ userId: 'user-123' })).rejects.toThrow(OperationNotAllowedError)
  })
