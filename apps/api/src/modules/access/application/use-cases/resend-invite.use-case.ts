  ...
  async execute(input: ResendInviteInput) {
    ...
    if (!input.scope.isGlobal && input.actorRole === 'MANAGER') {
      if (invite.invitedByUserId !== input.actorId) {
        throw new InsufficientPermissionsError(['invitation:resend'], [`invite:${invite.id}`])
      }
    }

    if (invite.status !== 'PENDING' || invite.revokedAt) {
      throw new OperationNotAllowedError('resend_invite', 'Only pending invitations can be resent')
    }

    if (invite.expiresAt < new Date()) {
      throw new OperationNotAllowedError('resend_invite', 'Invitation has expired')
    }
