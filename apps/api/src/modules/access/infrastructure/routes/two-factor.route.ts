...
function requireTwoFactorEnabled() {
  return new Elysia({ name: 'require-two-factor-enabled' }).onBeforeHandle(({ set }) => {
...
export const twoFactorRoute = new Elysia({
  ...
  .post(
    ...
    },
    {
      detail: {
        ...
        description: 'Generate a TOTP secret and otpauth URI for authenticator app enrollment.',
        tags: ['Authentication']
      },
      response: {
        200: t.Object({
  ...
  .post(
    ...
    },
    {
      detail: {
        summary: 'Disable 2FA',
        description: 'Disable two-factor authentication after verifying password and TOTP code.',
        tags: ['Authentication']
      },
      body: t.Object({
