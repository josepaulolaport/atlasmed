...
describe('userAssignmentsRoute', () => {
  ...
  beforeEach(() => {
    ...
    routeTestContext.mocks.assignUserManagerExecute.mockImplementation(() => Promise.resolve())
    routeTestContext.mocks.assignUserTerritoryExecute.mockReset()
    routeTestContext.mocks.assignUserTerritoryExecute.mockImplementation(() => Promise.resolve())
    routeTestContext.mocks.revokeUserTerritoryExecute.mockReset()
    routeTestContext.mocks.revokeUserTerritoryExecute.mockImplementation(() => Promise.resolve())
  })
  ...
  describe('GET /user/assignments', () => {
    it('returns authenticated user assignments for ADMIN', async () => {
      const app = createApp()
      const response = await app.handle(new Request('http://localhost/user/assignments'))
  ...
  describe('DELETE /users/:id/territories/:territoryId', () => {
    it('revokes territory for ADMIN', async () => {
      const app = createApp()
      const response = await app.handle(
        new Request(`http://localhost/users/${targetUserId}/territories/territory-a`, {
          method: 'DELETE'
        })
      )
