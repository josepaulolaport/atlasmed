...
const SUPPORTING_ROUTE_FILES = new Set(['refresh-cookie.ts', 'user.serializer.ts'])

describe('Access routes auth registry', () => {
  const routeFiles = readdirSync(ROUTES_DIR).filter((file) => file.endsWith('.route.ts'))
  ...
  it('documents route modules in registry sets', () => {
    const known = new Set([...PUBLIC_ROUTE_FILES, ...SUPPORTING_ROUTE_FILES])

    for (const file of routeFiles) {
      if (
        !known.has(file) &&
        !readFileSync(join(ROUTES_DIR, file), 'utf8').includes('.use(auth)')
      ) {
