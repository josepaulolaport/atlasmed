import { inviteUserSchema, type Role } from '@atlasmed/access'
...
export const inviteUserRoute = new Elysia({
  ...
  .post(
    '/invite',
    async ({ body, getUser, request, status }: any) => {
    ...
    },
    {
      detail: {
        ...
        description:
          'Create an invitation for a new user and deliver the invite token via email or WhatsApp. Requires admin permissions. Either email or phone number must be provided.',
        tags: ['Users'],
        security: [{ bearerAuth: [] }]
      },
      body: t.Object({
        ...
        managerTerritoryId: t.Optional(
          t.String({ description: 'Manager zone territory ID (required for MANAGER role)' })
        ),
        repTerritoryId: t.Optional(
          t.String({ description: 'Rep patch territory ID (required for REP role)' })
        )
      }),
      response: {
        200: t.Object({
