...
describe('AuthCacheService', () => {
  ...
  describe('invalidate', () => {
    test('should delete auth context and validation stamp from cache', async () => {
      ...
      expect(mockRedis.del).toHaveBeenCalledWith('auth:validated:user-123', 'auth:user:user-123')
    })

    test('should throw after Redis retries are exhausted', async () => {
      mockRedis.del = mock(() => Promise.reject(new Error('Redis error')))

      await expect(cache.invalidate('user-123')).rejects.toBeInstanceOf(RedisCacheError)
    })
  })
  ...
  describe('invalidateMultiple', () => {
    ...
    test('should throw after Redis retries are exhausted', async () => {
      mockRedis.del = mock(() => Promise.reject(new Error('Redis error')))
      await expect(cache.invalidateMultiple(['user-123'])).rejects.toBeInstanceOf(RedisCacheError)
    })
  })
  ...
  describe('validation helpers', () => {
    ...
    test('should mark auth context as validated', async () => {
      ...
      expect(mockRedis.setex).toHaveBeenCalledWith('auth:validated:user-123', 30, '1')
    })
  })
})
