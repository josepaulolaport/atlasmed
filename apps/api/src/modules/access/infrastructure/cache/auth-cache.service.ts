...
import type {
  CachedAuthContext,
  IAuthCache
} from '../../application/interfaces/auth-cache.interface'
...
export class AuthCacheService implements IAuthCache {
  ...
  async set(userId: string, context: CachedAuthContext): Promise<void> {
    try {
      await this.redis.setex(this.getKey(userId), this.ttl, JSON.stringify(context))
    } catch (error) {
  ...
  async invalidate(userId: string): Promise<void> {
    await withRedisRetry(() => this.redis.del(this.getValidatedKey(userId), this.getKey(userId)), {
  ...
  async invalidateMultiple(userIds: string[]): Promise<void> {
    ...
    const keys = userIds.flatMap((id) => [this.getValidatedKey(id), this.getKey(id)])
  ...
  async markValidated(userId: string): Promise<void> {
    try {
      await this.redis.setex(this.getValidatedKey(userId), AUTH_STATUS_REVALIDATION_SECONDS, '1')
    } catch (error) {
