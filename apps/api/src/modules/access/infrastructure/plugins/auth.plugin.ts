...
function assertActiveUserStatus(status: string, userId: string, ipAddress: string): void {
  if (status === 'ACTIVE') {
    return
  }
...
async function resolveAccessSessionFromToken(
  ...
  if (!session) {
    ...
    if (!dbSession) {
      logger.warn({ sessionId: payload.sid, userId: payload.sub, ipAddress }, 'Session not found')
      throw new UnauthorizedError()
    }
  ...
  if (!lastSeenCache) {
    sessionRepository.updateLastSeen(payload.sid).catch((error) => {
      logger.error({ sessionId: payload.sid, error: error.message }, 'Failed to update lastSeen')
    })
    ...
    redis.setex(lastSeenKey, 300, Date.now().toString()).catch((error) => {
      logger.error({ sessionId: payload.sid, error: error.message }, 'Failed to cache lastSeen')
    })
  } else {
...
export function createAuthPlugin(dependencies: AuthPluginDependencies) {
  ...
  return new Elysia({ name: 'auth' }).derive({ as: 'scoped' }, async ({ request }) => {
    ...
    try {
      ...
      return {
        ...
        getScope: async () => scopeService.resolve(authContext.userId, authContext.roleName),
        getUser: async () => {
        ...
        getAccessGrants: async () => accessGrantService.getActiveGrants(authContext.userId)
      }
    } catch (error) {
