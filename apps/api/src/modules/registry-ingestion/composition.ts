...
const registrySource = new MockRegistrySourceAdapter(environment.REGISTRY_MOCK_FIXTURE, fixturesDir)
...
async function acquireIngestionLock(): Promise<boolean> {
  const result = await redis.set(INGESTION_LOCK_KEY, '1', 'EX', INGESTION_LOCK_TTL_SECONDS, 'NX')
  return result === 'OK'
}
