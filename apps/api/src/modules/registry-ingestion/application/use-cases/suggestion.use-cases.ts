...
import { ConfigurationError, ForbiddenError, ValidationError } from '../../../../shared/errors'
...
function assertSuggestionInScope(scope: ScopeContext, suggestion: IngestionSuggestionRecord): void {
  if (scope.isGlobal) {
    return
  }
...
function resolveSuggestionFacilityScope(scope: ScopeContext): string[] | undefined {
  if (scope.isGlobal) {
    return undefined
  }
...
export class ApproveSuggestionUseCase {
  ...
  async execute(input: {
    ...
    const suggestion = await this.deps.suggestionRepository.findById(input.suggestionId)
    ...
    if (suggestion.status !== 'PENDING') {
      throw new ValidationError([{ field: 'status', message: 'Suggestion is not pending' }])
    }
    ...
    switch (suggestion.type) {
      ...
      case 'FACILITY_FIELD_UPDATE': {
        ...
        await this.deps.facilityRepository.applyApprovedFieldUpdates(suggestion.facilityId, updates)
      ...
      case 'PROFESSIONAL_FIELD_UPDATE': {
        ...
        if (!this.deps.professionalRepository) {
          throw new ConfigurationError(
            'professionalRepository is required to approve professional field updates'
          )
        }
      ...
      case 'FACILITY_REPRESENTATIVE_FIELD_UPDATE': {
        if (!this.deps.facilityRepresentativeRepository) {
          throw new ConfigurationError(
            'facilityRepresentativeRepository is required to approve representative suggestions'
          )
        }
        if (!suggestion.facilityId) {
          throw new ValidationError([
        ...
        await this.deps.facilityRepresentativeRepository.upsertFromRegistry({
          ...
          externalSourceKey,
          representativeName,
          roleTitle: typeof payload.roleTitle === 'string' ? payload.roleTitle : null,
      ...
      case 'FACILITY_REPRESENTATIVE_REMOVAL': {
        ...
        if (!externalSourceKey) {
          throw new ValidationError([
            {
              field: 'payload',
              message: 'Representative removal suggestion missing externalSourceKey'
            }
          ])
        }
