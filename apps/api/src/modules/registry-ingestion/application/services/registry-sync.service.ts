...
export class RegistrySyncService {
  ...
  async syncSnapshot(params: {
    ...
    const sourceAssociationKeys = new Set(
      snapshot.associations.map((a) => associationKey(a.doctorExternalId, a.clinicExternalId))
    )
    ...
    for (const row of activeSourceAssociations) {
      const key = associationKey(row.professionalExternalSourceId, row.facilityExternalSourceId)
