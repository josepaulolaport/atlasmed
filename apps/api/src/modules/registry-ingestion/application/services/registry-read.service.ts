...
export class RegistryReadService {
  ...
  async getRegistryFacility(input: { facilityId: string; scope: ScopeContext }) {
    const registryFacilityId = await this.resolveRegistryFacilityId(input.facilityId, input.scope)
    ...
    const projection =
      await this.deps.registryReadRepository.findFacilityByRegistryId(registryFacilityId)
  ...
  async getRegistryProfessionals(input: { facilityId: string; scope: ScopeContext }) {
    ...
    const professionals =
      await this.deps.registryReadRepository.findProfessionalsByFacility(registryFacilityId)
  ...
  async getRegistryRepresentatives(input: { facilityId: string; scope: ScopeContext }) {
    ...
    const representatives =
      await this.deps.registryReadRepository.findRepresentativesByFacility(registryFacilityId)
