...
export const sessionsRoute = new Elysia({
  ...
  .put(
    ...
    async ({ body, request, cookie }) => {
      ...
      const refreshToken = (cookieToken as string | undefined) || (bodyToken as string | undefined)
        ...
        if (result.error === 'cannot-revoke-current') {
          return status(400, {
            error: {
              ...
              message: 'Cannot revoke current session. Use DELETE /session/ instead.'
            }
          })
        }
      }
  ...
  .post(
    ...
    },
    {
      detail: {
        ...
        description: 'Revoke all sessions except the current one for the authenticated user.',
        security: [{ bearerAuth: [] }]
      },
      response: {
        200: t.Object({
