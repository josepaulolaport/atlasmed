...
function useCases(overrides: Partial<InteractionHttpUseCases> = {}): InteractionHttpUseCases {
  const read = { execute: mock(async () => ({ id: "interaction-1", status: "SCHEDULED", canMutate: true })) };
  const mutate = { execute: mock(async () => ({ id: "interaction-1", status: "IN_PROGRESS", version: 2 })) };
...
describe("Interaction HTTP routes", () => {
  it("passes actor and scope to GET /interactions/:id", async () => {
    const execute = mock(async () => ({ id: "interaction-1", status: "SCHEDULED" }));
    const response = await app(useCases({ get: () => ({ execute }) as never })).handle(new Request("http://localhost/interactions/interaction-1"));
    expect(response.status).toBe(200);
    expect(execute).toHaveBeenCalledWith(expect.objectContaining({ id: "interaction-1", actor: { userId: "rep-1", roleName: "REP" } }));
  });

  it("requires expectedVersion and Idempotency-Key for start and complete", async () => {
    const application = app(useCases());
    const missingKey = await application.handle(new Request("http://localhost/interactions/interaction-1/start", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ expectedVersion: 1 }),
    }));
    expect(missingKey.status).toBe(400);
    const missingVersion = await application.handle(new Request("http://localhost/interactions/interaction-1/complete", {
  ...
  it("passes a trimmed correction reason to completion", async () => {
    const execute = mock(async () => ({ id: "interaction-1", status: "COMPLETED" }));
    const response = await app(useCases({ complete: () => ({ execute }) as never })).handle(new Request("http://localhost/interactions/interaction-1/complete", {
  ...
  it("keeps managers read-only through INTERACTION permissions", async () => {
    const read = await app(useCases(), "MANAGER").handle(new Request("http://localhost/interactions/interaction-1"));
    expect(read.status).toBe(200);
    const write = await app(useCases(), "MANAGER").handle(new Request("http://localhost/interactions/interaction-1/start", {
