...
export function parseListFacilitiesQuery(query: Record<string, unknown>): ListFacilitiesQuery {
  ...
  if (
    ...
    (typeof commercialStatus !== 'string' ||
      !COMMERCIAL_STATUSES.includes(commercialStatus as FacilityCommercialStatus))
  )
    issues.push({ field: 'commercialStatus', message: 'commercialStatus is invalid' })
  if (query.productIds !== undefined && (!productIds || productIds.length === 0))
    issues.push({
      field: 'productIds',
      message: 'productIds must be a comma-separated list of product IDs'
    })
