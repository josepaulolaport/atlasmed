...
async function cleanupDemoData() {
  ...
  if (demoFacilityIds.length > 0 || demoProfessionalIds.length > 0) {
    const suggestionConditions = [
      ...
      ...(demoProfessionalIds.length > 0
        ? [inArray(cnesSuggestions.professionalId, demoProfessionalIds)]
        : [])
    ]
    ...
    const fpConditions = [
      ...(demoFacilityIds.length > 0
        ? [inArray(facilityProfessionals.facilityId, demoFacilityIds)]
        : []),
      ...(demoProfessionalIds.length > 0
        ? [inArray(facilityProfessionals.professionalId, demoProfessionalIds)]
        : [])
    ]
  ...
  if (demoUserIds.length > 0) {
    ...
    await db
      ...
      .where(inArray(userTerritoryAssignments.userId, demoUserIds))
...
async function seedFacilities(territorySet: { inScopePatchId: string; outOfScopePatchId: string }) {
