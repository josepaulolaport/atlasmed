  ...
  const steps: Array<{ label: string; run: () => Promise<{ count: number }> }> = [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const r = await db.$client.unsafe(`DELETE FROM public.territories WHERE id IN (${idList})`)
        return { count: getRawCount(r) }
      }
    },
    {
    ...
    },
    {
      ...
      run: async () => {
        const r = await db.$client.unsafe(`DELETE FROM public.users WHERE id != '${adminUserId}'`)
        return { count: getRawCount(r) }
      }
    }
  ]
...
function facilityImportWhereSql(filters: ImportFilters): string {
  ...
  if (filters.activeOnly) {
    clauses.push(`(f.deactivation_reason_code IS NULL OR trim(f.deactivation_reason_code) = '')`)
  }
...
async function syncPublicFromRegistry(options: ImportOptions) {
  ...
  if (options.dryRun) {
    console.log(
      '   (dry-run) Would bulk-insert facilities, professionals, associations, representatives.'
    )
    return
  }
  ...
  const scopedSync = await db.transaction(async (tx) => {
    ...
    const professionalsResult = await tx.execute(
      sql.raw(`
    ...
    console.log(
      `   ✓ professionals: ${professionalsInserted} rows (${Date.now() - professionalsInsertStartedAt}ms)`
    )

    const specialtyUpdateStartedAt = Date.now()
    const specialtiesResult = await tx.execute(
      sql.raw(`
    ...
    console.log(
      `   ✓ professional specialties: ${getRawCount(specialtiesResult)} rows (${Date.now() - specialtyUpdateStartedAt}ms)`
    )

    const crmUpdateStartedAt = Date.now()
    const crmResult = await tx.execute(
      sql.raw(`
      UPDATE public.professionals prof
    ...
    console.log(
      `   ✓ professional CRM fields: ${getRawCount(crmResult)} rows (${Date.now() - crmUpdateStartedAt}ms)`
    )

    const associationsInsertStartedAt = Date.now()
    const associationsResult = await tx.execute(
      sql.raw(`
    ...
    console.log(
      `   ✓ facility_professionals: ${associationsInserted} rows (${Date.now() - associationsInsertStartedAt}ms)`
    )

    const representativesInsertStartedAt = Date.now()
    const representativesResult = await tx.execute(
      sql.raw(`
    ...
    console.log(
      `   ✓ facility_representatives: ${representativesInserted} rows (${Date.now() - representativesInsertStartedAt}ms)`
    )

    const addressEnrichStartedAt = Date.now()
    const addressesResult = await tx.execute(
      sql.raw(`
    ...
    console.log(
      `   ✓ facility addresses enriched: ${getRawCount(addressesResult)} rows (${Date.now() - addressEnrichStartedAt}ms)`
    )
