...
function parseArgs(argv: string[]) {
  ...
  const stateFilter = stateArg ? (stateArg.split('=')[1]?.toUpperCase() as BrazilStateSigla) : null
...
async function loadMunicipalityMetadata(): Promise<Map<number, IbgeMunicipality>> {
  const municipalities = await fetchJson<IbgeMunicipality[]>(`${IBGE_LOCALIDADES_BASE}/municipios`)
