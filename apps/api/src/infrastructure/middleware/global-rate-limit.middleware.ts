...
import { type RateLimitConfig, rateLimiterService } from '../cache/rate-limiter.service'
...
export function createRateLimitMiddleware(namespace: string, options: RateLimitOptions) {
  return async (context: any) => {
    ...
    if (!result.allowed) {
      ...
      context.set.headers['retry-after'] = Math.ceil(retryAfterMs / 1000).toString()

      const createError =
        options.createError ?? ((retryAfter: number) => new RateLimitExceededError(retryAfter))
