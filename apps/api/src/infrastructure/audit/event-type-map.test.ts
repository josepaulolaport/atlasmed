...
describe("interaction audit event map", () => {
  it("maps lifecycle and occurrence commands without including sensitive content", () => {
    ...
    expect(resolveAuditEvent("PATCH", "/api/v1/calendar/calendar-1/occurrences/2026-08-03T09%3A00%5BUTC%5D")).toEqual({ eventType: "CALENDAR.OCCURRENCE_RESCHEDULED" });
    expect(resolveAuditEvent("DELETE", "/api/v1/calendar/calendar-1/occurrences/2026-08-03T09%3A00%5BUTC%5D")).toEqual({ eventType: "CALENDAR.OCCURRENCE_CANCELLED", severity: "WARNING" });
  });
});
