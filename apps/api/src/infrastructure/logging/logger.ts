...
import { createLogger, type LoggerContext, type ObservabilityLogger } from '@atlasmed/observability'
...
function wrapLogger(logger: ObservabilityLogger) {
  return {
    ...
    child(context: LogContext) {
      return wrapLogger({
        trace: (message, extra) => logger.trace(message, toLoggerContext({ ...context, ...extra })),
        debug: (message, extra) => logger.debug(message, toLoggerContext({ ...context, ...extra })),
        info: (message, extra) => logger.info(message, toLoggerContext({ ...context, ...extra })),
        warn: (message, extra) => logger.warn(message, toLoggerContext({ ...context, ...extra })),
