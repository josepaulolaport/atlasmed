      ...
      try {
        switch (name) {
          ...
          case 'cleanup-expired-permissions': {
            const { accessGrantService } = await import('../../modules/access/composition')
          ...
          case 'export-audit-logs-siem': {
            ...
            const since = cursorRaw ? new Date(cursorRaw) : new Date(Date.now() - 15 * 60 * 1000)
          ...
          case 'cleanup-old-audit-logs': {
            ...
            const deleted = await db
              .delete(auditLogs)
              .where(
                and(lt(auditLogs.createdAt, cutoffDate), inArray(auditLogs.severity, ['INFO']))
              )
