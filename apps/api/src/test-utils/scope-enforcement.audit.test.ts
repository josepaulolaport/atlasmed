...
import { SCOPE_ENFORCEMENT_EXEMPT, SCOPE_ENFORCEMENT_MANIFEST } from './scope-enforcement.manifest'
...
describe('scope enforcement audit (B.2)', () => {
  ...
  it('every scoped use case is manifested or explicitly exempt', () => {
    ...
    const unaccounted = scopedFiles.filter(
      (file) => !SCOPE_ENFORCEMENT_MANIFEST[file] && !SCOPE_ENFORCEMENT_EXEMPT.has(file)
    )
