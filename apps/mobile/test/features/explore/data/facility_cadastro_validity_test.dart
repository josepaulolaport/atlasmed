...
void main() {
  ...
  group('loadRequirementUploadLimits', () {
    test('reads the limits off the working-document response', () async {
      final client = _StubHttpClient(
        jsonEncode({
          ...
          'requirement': {
            ...
            'maxCombinedSizeBytes': 104857600,
          },
        }),
      );

      final limits = await FacilityCadastroRepository(
        7,
        client: client,
      ).loadRequirementUploadLimits(requirementId: 5);

      expect(limits!.maxFiles, 2);
      expect(limits.maxFileSizeBytes, 52428800);
      expect(limits.allowsImages, isFalse);
    });

    test('a response without a requirement block yields null limits', () async {
      final client = _StubHttpClient(jsonEncode({'id': 300}));
...
class _StubHttpClient extends RepositoryHttpClient {
  _StubHttpClient(this.body);
  ...
  Future<RepositoryHttpResponse> call({
    ...
    return RepositoryHttpResponse(statusCode: 200, headers: {}, body: body);
  }
}
