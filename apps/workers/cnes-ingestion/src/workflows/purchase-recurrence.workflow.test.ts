...
describe("purchase recurrence workflow", () => {
  ...
  test("continues as new with the identical computed window, sweep decision, source cursor, and totals", async () => {
    ...
    await runPurchaseRecurrenceWorkflow({ mode: "RECONCILE", cursor: "source-cursor" }, {
      recalculate: async (input) => {
        ...
        cursor += 500;
        return { processed: 500, updated: 400, failed: 0, nextCursor: `f-${cursor}`, failures: [] };
      },
      rebuild: async () => {},
      logLifecycle: async () => {},
      continueAsNew,
    ...
    expect(continuedInputs[0]).toMatchObject({
      ...
      cursor: "f-10000", sourceCursor: "source-cursor", totals: { processed: 10000, updated: 8000, failed: 0 }, lifecycleStartedAt: "2026-07-22T00:00:00.000Z",
    });
  });
