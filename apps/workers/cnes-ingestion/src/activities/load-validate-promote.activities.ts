...
export async function validateStagingActivity(input: {
  ...
  if (!report.passed) {
    throw new Error(
      `Staging validation failed: ${JSON.stringify(report.checks.filter((c) => !c.passed))}`
    )
  }
...
export async function promoteRegistrySwapActivity(input: {
  ...
  await db.update(cnesRuns).set({ promotedAt }).where(eq(cnesRuns.id, input.ingestionRunId))
