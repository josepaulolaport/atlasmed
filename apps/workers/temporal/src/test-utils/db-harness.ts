 ...
 *
 * The first character is a digit, and that is load-bearing: every real UF is
 * two letters, so a digit makes a collision with one impossible. Drawing it
 * from the full alphabet meant the very first call could return `PA` or `MS`,
 * which passes on an empty database and fails on a production clone — the
 * fixture then looks flaky rather than wrong.
 */
