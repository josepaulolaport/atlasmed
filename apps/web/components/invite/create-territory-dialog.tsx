...
export function CreateTerritoryDialog({
  ...
  const onSubmit = async (data: CreateTerritoryFormData) => {
    ...
    try {
      ...
      setError(error.response?.data?.error || 'Falha ao criar território')
    } finally {
  ...
  const title =
    territoryType === 'manager_zone' ? 'Criar Zona de Gerente' : 'Criar Território de Representante'
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-500 text-xs">Código único para identificar este território</p>
          </div>
          ...
          <div className="rounded-md bg-blue-50 p-3 text-blue-700 text-sm">
            ...
            <p className="mt-1">
              A definição de limite (boundary) pode ser configurada posteriormente na página de
              territórios. Não é necessário para atribuição.
            </p>
          </div>

          <div className="flex gap-3 pt-2">
            <Button type="submit" disabled={isSubmitting || loadingTypes} className="flex-1">
              {isSubmitting ? (
                <>
