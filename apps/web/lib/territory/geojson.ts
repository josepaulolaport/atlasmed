...
export function boundaryToPolygonCoordinates(boundary: GeoJsonPolygon): PolygonCoordinates[] {
  if (boundary.type === 'Polygon') {
    return [boundary.coordinates as PolygonCoordinates]
  }

  return (boundary.coordinates as MultiPolygonCoordinates).filter((polygon) => polygon.length > 0)
}

/** Merge one or more polygons into a Polygon or MultiPolygon boundary. */
export function combinePolygonCoordinates(polygons: PolygonCoordinates[]): GeoJsonPolygon | null {
...
export function drawFeaturesToBoundary(features: GeoJSON.Feature[]): GeoJsonPolygon | null {
