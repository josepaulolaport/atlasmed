...
export function getTerritoryAssignmentPickerConfig(
  ...
  if (role === 'REP') {
    return {
      ...
      helperText: 'Field reps are assigned to territories whose type allows clinic assignment.'
    }
  }
  ...
  return {
    ...
    excludeCountry: true,
    helperText: 'Managers are assigned to territory types configured for manager oversight.'
  }
}
...
export function formatTerritoryLabel(
  t: Pick<Territory, 'code' | 'name' | 'slug' | 'territoryType' | 'countryCode' | 'isCountryLevel'>
): string {
