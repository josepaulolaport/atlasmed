...
export const mapsApi = {
  ...
  async forwardGeocode(query: string, options?: { country?: string; limit?: number }) {
    const response = await apiClient.get<{ data: GeocodeResult | null }>('/maps/geocode/forward', {
      params: {
        q: query,
  ...
  async reverseGeocode(longitude: number, latitude: number) {
    const response = await apiClient.get<{ data: GeocodeResult | null }>('/maps/geocode/reverse', {
