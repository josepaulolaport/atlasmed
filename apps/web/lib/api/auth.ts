...
export const authApi = {
  ...
  revokeOtherSessions: async (): Promise<{ revokedCount: number }> => {
    const response = await apiClient.post<{ revokedCount: number }>('/session/revoke-others')
    return response.data
  },
  ...
  verify2FALogin: async (data: { pendingToken: string; code: string }): Promise<LoginResponse> => {
  ...
  getCapabilities: async (): Promise<CapabilitiesResponse> => {
    const response = await apiClient.get<CapabilitiesResponse>('/access/me/capabilities')
    return response.data
  },
  ...
  confirm2FA: async (data: { code: string }): Promise<{ success: boolean }> => {
    const response = await apiClient.post<{ success: boolean }>('/access/2fa/confirm', data)
    return response.data
  },

  disable2FA: async (data: { password: string; code: string }): Promise<{ success: boolean }> => {
    const response = await apiClient.post<{ success: boolean }>('/access/2fa/disable', data)
    return response.data
  }
}
