...
import type { Invitation, InviteUserRequest, RoleInfo, User, UserAssignments } from '@/types/auth'
...
export const usersApi = {
  ...
  getUserAssignments: async (userId: string): Promise<UserAssignments> => {
    const response = await apiClient.get<UserAssignments>(`/access/users/${userId}/assignments`)
    return response.data
  },

  assignManager: async (userId: string, managerId: string | null): Promise<{ message: string }> => {
    const response = await apiClient.patch<{ message: string }>(`/access/users/${userId}/manager`, {
      managerId
    })
    return response.data
  },

  assignTerritory: async (userId: string, territoryId: string): Promise<{ message: string }> => {
  ...
  revokeTerritory: async (userId: string, territoryId: string): Promise<{ message: string }> => {
  ...
  assignSector: async (userId: string, sectorId: string): Promise<{ message: string }> => {
    const response = await apiClient.post<{ message: string }>(`/access/users/${userId}/sectors`, {
      sectorId
    })
    return response.data
  },
