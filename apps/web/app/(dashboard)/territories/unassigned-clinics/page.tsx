...
import type { UnassignedClinic } from "@/types/territory";

export default function UnassignedClinicsPage() {
  const { user } = useAuth();
  ...
  const [overrideClinic, setOverrideClinic] = useState<UnassignedClinic | null>(null);
  ...
  const loadClinics = useCallback(async () => {
    setLoading(true);
    try {
      const response = await territoriesApi.listUnassignedClinics({ page, limit: 20 });
  ...
  const handleOverride = async () => {
    if (!overrideClinic || !overrideTerritoryId) return;
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Clinic territory overridden",
        variant: "success",
      });
  ...
  const handleUnlock = async (clinicId: string) => {
    setUnlockingId(clinicId);
    try {
      await territoriesApi.unlockClinicGeo(clinicId);
      toast({
        title: "Success",
        description: "Clinic geo lock removed",
        variant: "success",
      });
      await loadClinics();
    } catch (err) {
