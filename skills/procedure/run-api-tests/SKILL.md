...
category: procedure
scope: api
description: Boot the api test environment (PostgreSQL, Redis, atlasmed_test DB, migrations, seed) and run tests. Covers automated setup, manual setup, unit/integration/watch modes, DB reset, troubleshooting. Extracted from apps/api/TESTING.md.
appliesTo:
...
## Attach when
- Task runs or writes tests under `apps/api`.
- Task creates a new test file that needs the database or Redis.
- Task troubleshoots a failing test suite.
...
- `apps/api/TESTING.md` — trim to a pointer + link to this skill.
