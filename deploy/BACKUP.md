...
- The `Deploy Backend Infrastructure` workflow creates the configured `STORAGE_BUCKET` and `CNES_ARCHIVE_S3_BUCKET` buckets with an ephemeral `minio/mc` container after `atlasmed-minio` deploys.
