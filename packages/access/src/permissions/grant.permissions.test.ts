...
import { canAccessResource, canAccessRoute } from './route.permissions'
...
describe('defineAbilitiesForUser', () => {
  ...
  it('allows route-level facility update for USER via role permissions', () => {
    ...
    expect(canAccessResource(Role.REP, grants, 'update', 'FACILITY', 'facility-1')).toBe(true)
  })
  ...
  it('maps legacy CLINIC grants to FACILITY subject', () => {
    ...
    expect(canOnResource(withoutGrant, 'delete', 'FACILITY', 'facility-1')).toBe(false)
...
describe('route permission helpers', () => {
  ...
  it('canAccessResource allows USER facility update for matching and other ids', () => {
    ...
    expect(canAccessResource(Role.REP, grants, 'update', 'FACILITY', 'facility-1')).toBe(true)
    expect(canAccessResource(Role.REP, grants, 'update', 'FACILITY', 'facility-2')).toBe(true)
  })
})
