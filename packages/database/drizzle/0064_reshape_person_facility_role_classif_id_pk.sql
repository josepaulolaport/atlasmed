CREATE TABLE "person_facility_classification_assignments" (
	...
	CONSTRAINT "person_facility_classification_assignments_pkey" PRIMARY KEY("person_facility_id","classification_id")
);
--> statement-breakpoint
CREATE TABLE "person_facility_classifications" (
	"id" bigint PRIMARY KEY GENERATED ALWAYS AS IDENTITY (sequence name "person_facility_classifications_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1),
	"code" text NOT NULL,
	"name" text NOT NULL,
	"is_active" boolean DEFAULT true NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "person_facility_classifications_code_key" UNIQUE("code")
);
--> statement-breakpoint
...
CREATE TABLE "person_facility_roles" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "person_facility_roles_code_key" UNIQUE("code")
);
--> statement-breakpoint
...
ALTER TABLE "person_facility_classification_assignments" ADD CONSTRAINT "person_facility_classification_assignments_classification_id_person_facility_classifications_id_fk" FOREIGN KEY ("classification_id") REFERENCES "public"."person_facility_classifications"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "person_facility_role_assignments" ADD CONSTRAINT "person_facility_role_assignments_role_id_person_facility_roles_id_fk" FOREIGN KEY ("role_id") REFERENCES "public"."person_facility_roles"("id") ON DELETE restrict ON UPDATE no action;