...
export const facilities = pgTable(
  'facilities',
  {
    ...
    // --- Classification ---
    primarySectorId: text('primary_sector_id').references(() => sectors.id, {
      onDelete: 'set null'
    }),
    ...
    territoryAssignmentStatus: territoryAssignmentStatusEnum('territory_assignment_status')
      .notNull()
      .default('unassigned'),
    territoryAssignmentSource: territoryAssignmentSourceEnum('territory_assignment_source')
      .notNull()
      .default('geo'),
  ...
  (t) => [
    uniqueIndex('facilities_source_provider_external_source_id_uidx').on(
      t.sourceProvider,
      t.externalSourceId
    ),
...
export const professionals = pgTable(
  ...
  (t) => [
    uniqueIndex('professionals_source_provider_external_source_id_uidx').on(
      t.sourceProvider,
      t.externalSourceId
    ),
...
export const facilityHealthcareProviderShares = pgTable(
  'facility_healthcare_provider_shares',
  {
    ...
    facilityId: text('facility_id')
      .notNull()
      .references(() => facilities.id, { onDelete: 'cascade' }),
    healthcareProviderId: text('healthcare_provider_id')
      .notNull()
  ...
  (t) => [
    ...
    index('facility_healthcare_provider_shares_healthcare_provider_id_idx').on(
      t.healthcareProviderId
    )
  ]
)
...
export const conformityRecords = pgTable(
  ...
  (t) => [
    uniqueIndex('conformity_records_facility_id_requirement_id_uidx').on(
      t.facilityId,
      t.requirementId
    ),
...
export const facilityProfessionalsRelations = relations(facilityProfessionals, ({ one }) => ({
  professional: one(professionals, {
    fields: [facilityProfessionals.professionalId],
    references: [professionals.id]
  }),
  facility: one(facilities, {
    fields: [facilityProfessionals.facilityId],
...
export const facilityRepresentativesRelations = relations(facilityRepresentatives, ({ one }) => ({
  facility: one(facilities, {
    fields: [facilityRepresentatives.facilityId],
    references: [facilities.id]
  }),
  confirmedBy: one(users, {
...
export const facilityConsultantAssignmentsRelations = relations(
  facilityConsultantAssignments,
  ({ one }) => ({
    facility: one(facilities, {
      fields: [facilityConsultantAssignments.facilityId],
      references: [facilities.id]
    }),
    user: one(users, {
...
export const facilityHealthcareProviderSharesRelations = relations(
  facilityHealthcareProviderShares,
  ({ one }) => ({
    facility: one(facilities, {
      fields: [facilityHealthcareProviderShares.facilityId],
      references: [facilities.id]
    }),
...
export const conformityRequirementsRelations = relations(
...
export const conformityRecordsRelations = relations(conformityRecords, ({ one }) => ({
  facility: one(facilities, {
    fields: [conformityRecords.facilityId],
    references: [facilities.id]
  }),
