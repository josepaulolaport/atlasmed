...
export const territoryTypes = pgTable(
  'territory_types',
  {
    ...
    participatesInGroupingHierarchy: boolean('participates_in_grouping_hierarchy')
      .notNull()
      .default(false),
...
export const territories = pgTable(
  'territories',
  {
    ...
    territoryTypeId: text('territory_type_id')
      .notNull()
      .references(() => territoryTypes.id, { onDelete: 'restrict' }),
...
export const territoryApprovalRequests = pgTable(
  'territory_approval_requests',
  {
    ...
    targetTerritoryId: text('target_territory_id').references(() => territories.id, {
      onDelete: 'set null'
    }),
    facilityId: text('facility_id'),
    toTerritoryId: text('to_territory_id').references(() => territories.id, {
      onDelete: 'set null'
    }),
    reason: text('reason'),
  ...
  (t) => [
    ...
    index('territory_approval_requests_target_territory_id_status_idx').on(
      t.targetTerritoryId,
      t.status
    ),
    index('territory_approval_requests_facility_id_status_type_idx').on(
      t.facilityId,
      t.status,
      t.type
    )
...
export const territoriesRelations = relations(territories, ({ one, many }) => ({
  ...
  facilityApprovalRequests: many(territoryApprovalRequests, {
...
export const userTerritoryAssignmentsRelations = relations(userTerritoryAssignments, ({ one }) => ({
  ...
  territory: one(territories, {
    fields: [userTerritoryAssignments.territoryId],
    references: [territories.id]
  })
}))

export const territoryApprovalRequestsRelations = relations(
