...
import { index, integer, json, pgSchema, text, timestamp } from 'drizzle-orm/pg-core'
import { facilities, facilityProfessionals, professionals } from '../public/facilities'
...
export const cnesDiffScopeEnum = ingestionSchema.enum('cnes_diff_scope', ['WAREHOUSE', 'CRM'])
...
export const cnesDiffs = ingestionSchema.table(
  'cnes_diffs',
  {
    id: text('id')
      .primaryKey()
      .$defaultFn(() => createId()),
    cnesRunId: text('cnes_run_id')
      .notNull()
      .references(() => cnesRuns.id, { onDelete: 'cascade' }),
...
export const cnesSuggestions = ingestionSchema.table(
  'cnes_suggestions',
  {
    ...
    facilityProfessionalId: text('facility_professional_id').references(
      () => facilityProfessionals.id,
      { onDelete: 'set null' }
    ),
    reason: text('reason'),
    payload: json('payload').notNull().default({}),
