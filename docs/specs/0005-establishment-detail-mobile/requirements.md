...
> **Order creation deferral note (2026-08-02):** order history remains available in the main **Pedidos** page and in each establishment detail. Only actions that start a new order are temporarily unmounted: the establishment **Pedido** quick action, the **Novo pedido** button, the empty-history **Criar pedido** action, and **Repetir pedido**. Creation routes, screens, repositories, models, and API integration remain in the codebase for later reactivation.
