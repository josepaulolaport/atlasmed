...
- **Reconcile runs off a watermark.** `since` was a fixed two-hour lookback from each run's own start, which under overlap `SKIP` left the hours after an overrunning run covered by nobody. `ops.purchase_recurrence_watermark` records how far a *completed* run reached; a failed run re-covers its window. A watermark more than 24 hours behind escalates to a sweep rather than widening the window.
...
- Rollout order is generated migration, compatible API/worker deployment, schedule provisioning, initial backfill, aggregate/drift checks, and facilities search rebuild. **The rebuild is not optional after the 2026-08-15 amendment**: `purchaseIntervalDaysMax`, `purchaseIntervalSourcesAny` and `manualPurchaseProfilesAny` are absent from every existing document, and a Meili filter on a field a document lacks simply does not match it.
